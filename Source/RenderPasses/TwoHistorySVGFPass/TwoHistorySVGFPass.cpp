...
namespace
{
    ...
    const char kTemporalFilterShader[] = "RenderPasses/TwoHistorySVGFPass/TemporalFilter.ps.slang";
    const char kAtrousShader[] = "RenderPasses/TwoHistorySVGFPass/Atrous.ps.slang";
    const char kFilterMomentShader[] = "RenderPasses/TwoHistorySVGFPass/FilterMoments.ps.slang";
    const char kReprojectShader[] = "RenderPasses/ReprojectionPass/Reprojection.ps.slang";
    const char kBlendingShader[] = "RenderPasses/TwoHistorySVGFPass/Blending.ps.slang";
    ...
    const char kFilterGradientShader[] = "RenderPasses/TwoHistorySVGFPass/FilterGradient.ps.slang";

    // Names of valid entries in the parameter dictionary.
    const char kEnabled[] = "Enabled";
    const char kSpatialFilterEnabled[] = "SpatialFilterEnabled";
    const char kTwoHistoryEnabled[] = "TwoHistoryEnabled";
    ...
    enum BlendingOutFields
    {
        BlendingOutFields_Color,
        BlendingOutFields_Gamma,
    };
    ...
    : RenderPass(kInfo)
{
    for (const auto& [key, value] : dict)
    {
        ...
        else if (key == kTwoHistoryEnabled) mTwoHistoryEnabled = value;
...
Dictionary TwoHistorySVGFPass::getScriptingDictionary()
{
    ...
    dict[kTwoHistoryEnabled] = mTwoHistoryEnabled;
...
uint32_t TwoHistorySVGFPass::getReprojectStructSize()
{
...
void TwoHistorySVGFPass::execute(RenderContext* pRenderContext, const RenderData& renderData)
{
    ...
    */
    ...
    if (!mpPackLinearZAndNormal || mRecompile)
    {
        ...
        defines.add("_DW_ENABLED", mTwoHistoryEnabled ? "1" : "0");
        defines.add("_DEBUG_TAG_ENABLED", mEnableDebugTag ? "1" : "0");
        defines.add("_DEBUG_OUTPUT_ENABLED", mEnableDebugOutput ? "1" : "0");
        defines.add("_BEST_GAMMA_ENABLED", mTwoHistoryEnabled && mBestGammaEnabled ? "1" : "0");
        defines.add("_OPTIMAL_WEIGHTING_ENABLED", mTwoHistoryEnabled && mOptinalWeightingEnabled ? "1" : "0");
        ...
        mpBlending = FullScreenPass::create(kBlendingShader, defines);
...
{
    {
        ...
        desc.setColorTarget(TemporalFilterOutFields_HistoryWeight,
                            mTwoHistoryEnabled ? Falcor::ResourceFormat::RG32Float : Falcor::ResourceFormat::R32Float);
    ...
    {
        Fbo::Desc desc;
        desc.setColorTarget(BlendingOutFields_Color, Falcor::ResourceFormat::RGBA32Float);
        for (int i=0; i<4; i++)
        {
...

    {
        Fbo::Desc desc;
        desc.setColorTarget(BlendingOutFields_Color, Falcor::ResourceFormat::RGBA32Float);
        desc.setColorTarget(BlendingOutFields_Gamma, Falcor::ResourceFormat::R32Float);
        mpBlendingFbo = Fbo::create2D(dim.x, dim.y, desc);
    }
...
void TwoHistorySVGFPass::clearBuffers(RenderContext* pRenderContext, const RenderData& renderData)
{
    ...
    pRenderContext->clearFbo(mpBlendingFbo.get(), float4(0), 1.0f, 0, FboAttachmentType::All);
...
void TwoHistorySVGFPass::computeReprojection(RenderContext* pRendercontext,
...
void TwoHistorySVGFPass::computeTemporalFilter(RenderContext* pRenderContext,
...
void TwoHistorySVGFPass::computeFilterGradient(RenderContext* pRenderContext, const RenderData& renderData)
{
...
void TwoHistorySVGFPass::computeAtrousDecomposition(RenderContext* pRenderContext, const RenderData& renderData, Texture::SharedPtr pAlbedoTexture)
{
    ...
    if (mFeedbackTap < 0)
    {
        ...
        if (mTwoHistoryEnabled)
        {
            ...
            blending(pRenderContext, renderData,
                mpPingPongFbo[0]->getColorTexture(0),
                mpPingPongFbo[2]->getColorTexture(0),
                mpBlendingFbo);

            pRenderContext->blit(mpBlendingFbo->getColorTexture(BlendingOutFields_Color)->getSRV(),
    ...
    for (int i = 0; i < mFilterIterations; i++)
    {
        ...
        int nSrcIds = (mTwoHistoryEnabled && !doneSelection) ? 4 : 2;
        // int nSrcIds = 4;
        ...
        if (mTwoHistoryEnabled && i == mFeedbackTap)
        {
            blending(pRenderContext, renderData,
                mpPingPongFbo[0]->getColorTexture(0),
                mpPingPongFbo[2]->getColorTexture(0),
...
void TwoHistorySVGFPass::blending(
    ...
    Fbo::SharedPtr pOutputFbo)
{
    ...
    // cerr << "blending()" << endl;
    auto perImageCB = mpBlending["PerImageCB"];
    ...
    mpBlending->execute(pRenderContext, pOutputFbo);

    if (mEnableDebugOutput)
    {
        FALCOR_PROFILE("[debug] blit debug outputs");
        pRenderContext->blit(pOutputFbo->getColorTexture(BlendingOutFields_Gamma)->getSRV(), pOutputGammaTexture->getRTV());
    }
}
...
void TwoHistorySVGFPass::renderUI(Gui::Widgets& widget)
{
    ...
    dirty |= widget.checkbox("Enable Dynamic Weighting", mTwoHistoryEnabled);
    if (mTwoHistoryEnabled)
    {
