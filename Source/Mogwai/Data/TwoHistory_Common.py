...
def migrateToNewKeys(graph_params: dict) -> dict:
    if 'dynamic_weighting_enabled' in graph_params:
        graph_params['two_history_enabled'] = graph_params['dynamic_weighting_enabled']
        graph_params.pop('dynamic_weighting_enabled')

    if 'dynamic_weighting_params' in graph_params:
        graph_params['two_history_params'] = graph_params['dynamic_weighting_params']
        graph_params.pop('dynamic_weighting_params')
...
def normalizeGraphParams(graph_params: dict) -> dict:
    ...
    graph_params = {**default_params, **migrateToNewKeys(graph_params)}
    ...
    if not graph_params['two_history_enabled']:
        graph_params['grad_iters'] = 0
        if 'two_history_params' in graph_params:
            pop_keys = ['SelectionMode', 'GradientMidpoint', 'GammaSteepness', 'WeightedAlpha', 'GradientAlpha', 'NormalizationMode']
            popKeys(graph_params['two_history_params'], pop_keys)
    elif graph_params['two_history_params']['SelectionMode'] == SelectionMode.UNWEIGHTED:
        graph_params['grad_iters'] = 0
        pop_keys = ['GradientMidpoint', 'GammaSteepness', 'WeightedAlpha', 'GradientAlpha', 'NormalizationMode']
        popKeys(graph_params['two_history_params'], pop_keys)
    elif graph_params['two_history_params']['SelectionMode'] == SelectionMode.WEIGHTED:
