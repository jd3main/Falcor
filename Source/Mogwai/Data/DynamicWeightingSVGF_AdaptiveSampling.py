...
def render_graph_g():
    ...
    DynamicWeightingSVGF = createPass('DynamicWeightingSVGF', {'Enabled': True, 'DynamicWeighingEnabled': True, 'Iterations': 1, 'FeedbackTap': -1, 'GradientFilterIterations': 0, 'VarianceEpsilon': 9.999999747378752e-05, 'PhiColor': 10.0, 'PhiNormal': 128.0, 'Alpha': 0.05000000074505806, 'MomentsAlpha': 0.20000000298023224, 'GradientAlpha': 0.20000000298023224, 'GradientMidpoint': 0.009999999776482582, 'GammaSteepness': 100.0, 'SelectionMode': 2, 'SampleCountOverride': -1, 'NormalizationMode': 3, 'OutputPingPongAfterIters': 0, 'OutputPingPongIdx': 0, 'EnableDebugTag': False, 'EnableDebugOutput': True})
    ...
    AdaptiveSampling = createPass('AdaptiveSampling', {'MaxVariance': 10.0, 'AverageSampleCountBudget': 2.0})
    g.addPass(AdaptiveSampling, 'AdaptiveSampling')
    ColorMapPass3 = createPass('ColorMapPass', {'colorMap': ColorMap.Plasma, 'channel': 0, 'autoRange': False, 'minValue': 0.0, 'maxValue': 16.0})
    ...
    g.addEdge('DynamicWeightingSVGF.Variance', 'AdaptiveSampling.var')
    g.addEdge('DynamicWeightingSVGF.HistLength', 'AdaptiveSampling.histLength')
    g.addEdge('AdaptiveSampling.sampleCount', 'ColorMapPass3.input')
    g.addEdge('AdaptiveSampling.sampleCount', 'CountToColor.Count')
    g.addEdge('SharedBuffer.buffer', 'PathTracer.sampleCount')
    g.addEdge('SharedBuffer.buffer', 'AdaptiveSampling.sampleCountSharedBuffer')
    g.addEdge('SharedBuffer.buffer', 'ColorMapPass2.input')
    g.addEdge('SharedBuffer.buffer', 'DynamicWeightingSVGF.SampleCount')
