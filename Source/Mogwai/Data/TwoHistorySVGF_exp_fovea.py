...
def render_graph_g(iters, feedback, alpha=0.05,
                   ...
                   two_history_enabled=False, two_history_params:dict={},
    ...
    SVGFPass = createPass('TwoHistorySVGFPass', {
        'Enabled': svgf_enabled,
        'SpatialFilterEnabled': spatial_filter_enabled,
        'TwoHistoryEnabled': two_history_enabled,
        'Iterations': iters,
...
def getOutputFolderName(scene_name: str, graph_params: dict, debug=False) -> Path:
    ...
    if graph_params['two_history_enabled']:
        th_params = graph_params['two_history_params']

        if 'FilterGradientEnabled' in th_params and th_params['FilterGradientEnabled']:
            folder_name_parts.append('FG')

        if 'BestGammaEnabled' in th_params and th_params['BestGammaEnabled']:
            folder_name_parts.append('BG')

        selection_mode = th_params['SelectionMode']
        if selection_mode == SelectionMode.LINEAR:
            folder_name_parts.append('Linear({},{})'.format(
                th_params['GradientMidpoint'],
                th_params['GammaSteepness']))
...
for scene_idx, scene_path in enumerate(scene_paths):
    ...
    for iters, feedback in iter_params:
        ...
        for midpoint, steepness in blending_func_params:
            logI(f"Run Two History: iters={iters}, feedback={feedback}, midpoint={midpoint}, steepness={steepness}")
            run(graph_params = {
                    'iters': iters,
                    'feedback': feedback,
                    'two_history_enabled': True,
                    'two_history_params': {
