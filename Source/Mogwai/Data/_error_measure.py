...
## pre-check
n_ref_images = countImages(record_path/f'{scene_name}_iters({iters})', f'{fps}fps.SVGFPass.Filtered image.{{}}.exr')
...
midpoints = [0.0, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1.0]
steepnesses = ([0,] if selection_func=='Step' else [0.5, 5.0, 50.0, 500.0, 5000.0])
...
source_folders.append(record_path/f'{scene_name}_iters({iters})_Foveated(SPLIT_HORIZONTALLY,SHM,8.0)')
source_folders.append(record_path/f'{scene_name}_iters({iters})_Weighted_Foveated(SPLIT_HORIZONTALLY,SHM,8.0)')
...
st = time.time()
fields = ['mean', '25-th percentile', '50-th percentile', '75-th percentile', '99-th percentile', 'top-1-percent']
# fields = ['mean']
...
for folder_index, folder in enumerate(source_folders):
    ...
    for i, source_image in enumerate(source_images):
        ...
        results[folder_index, i, :] = xp.array([
            xp.mean(err),
            *percentiles,
            top_1_percent,
        ])
    ...
    avg_mean_err = avg_results[folder_index, 0]
    ...
    for field_indx, field in enumerate(fields):
        ...
        np.savetxt(save_path, results[folder_index, :, field_indx])
ed = time.time()
...
save_path = Path(f'{scene_name}_iters({iters})_fps({fps})_t({record_seconds})_{selection_func}_ErrorMeasure.txt')
...
if PLOT_ERROR_OVER_TIME:
    ...
    fig.set_label(f'{scene_name} iters({iters})')
